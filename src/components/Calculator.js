...
function TypeOfCustomer(){
    return(
        <div className={'d-grid gap-2 d-md-flex  justify-content-between  '}>
                <button className='btn btn-link d-flex justify-content-center'>New Customer</button>
                <button className='btn btn-link d-flex justify-content-center'>Returning Customer</button>
         </div>
    )
}
function Header(){
...
const Calculator = () => {
    ...
    const [amountRecievable,setAmountRecievable] = useState(calcModel.amountRecievable);
    ...
    return (
        <AmountContext.Provider value={updateAmount}>
            <div className={'card'} style={{width:"35rem"}}>
                ...
                <Estimates serviceFee={serviceFee} amountRecievable={amountRecievable} repayment={repayment} />
                <SubmitButton />
            </div>
            </div>
